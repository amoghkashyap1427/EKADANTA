...
class KailashWorldScene(BaseScene):
    def __init__(self, game):
        ...
        self.shiva_attack_cooldown = 1.5
    ...
    def update(self, dt: float):
        ...
        if not self.dialogue_manager.active:
            ...
            if self.state == "VIGHNA_INTRO":
                ...
                self.state_timer += dt
                if self.state_timer > 0.35 and self.state_timer < 0.45:
                    dialogue = [
                ...
                # Narrative survival battle: Shiva cannot be defeated and this
                # sequence must always reach its scripted conclusion.
                self.player.health = max(1, self.player.health)
                self.gate_integrity = max(1.0, self.gate_integrity)
                ...
                if self.state_timer > 5.0:
                    ...
                    def on_end():
                        ...
                        self.set_objective("PROTECT THE GATE")
                        self.battle_timer = 60.0
                        self.shiva_attack_cooldown = 1.0
                        self.power_moment_triggered = False
                        self.player.health = max(1, self.player.health)
                    self.dialogue_manager.start_dialogue(dialogue, on_end)
                ...
                if self.state_timer > 1.0 and self.state_timer < 1.1:
                    ...
                    def on_end():
                        self.state = "FINAL_STAND_BATTLE"
                        self.state_timer = 0.0
                        self.shiva_attack_cooldown = max(0.5, self.shiva_attack_cooldown)
                        self.set_objective("PROTECT THE GATE")
                    self.dialogue_manager.start_dialogue(dialogue, on_end)
                ...
                self.state_timer += dt
                # Give the reborn form a brief, peaceful moment, then conclude.
                if self.state_timer > 8.0:
                ...
                self.state_timer += dt
                # IMPORTANT: change_scene() starts a fade transition. While that
                # transition is running, this scene continues receiving update()
                # calls. Guard the call so we do not restart the fade every frame
                # and remain stuck on a black screen forever.
                if (
                    self.state_timer > 26.0
                    and not self.game.scene_manager.transitioning
                ):
                    from src.scenes.main_menu import MainMenuScene
                    self.game.scene_manager.change_scene(MainMenuScene(self.game))
                    self.state = "ENDING_TO_MENU"
    ...
    def draw(self, surface: pygame.Surface):
        ...
        if self.state == "VIGHNA_INTRO":
            ...
            darken = max(0.0, 1.0 - min(1.0, self.state_timer / 1.5))
        ...
        if self.state in ["CLIFFHANGER", "CHAPTER_2_INTRO", "BROKEN_GATE_CLIFFHANGER", "GANESHA_FALLEN", "THE_INEVITABLE_MOMENT", "CHAPTER_2_ENDING", "SHIVA_REALIZATION", "PARVATI_ARRIVES", "CHAPTER_3_INTRO", "SHIVA_PROMISE", "DIVINE_RESTORATION", "ELEPHANT_REVEAL", "GANESHA_IDENTITY", "FINAL_ENDING_CARDS"]:
            if self.state == "CHAPTER_2_INTRO":
                alpha = max(0, 255 - int(self.state_timer * 170))
            elif self.state == "CHAPTER_3_INTRO":
                alpha = max(0, 255 - int(self.state_timer * 170))
            else:
            ...
            if self.state == "BROKEN_GATE_CLIFFHANGER":
                ...
                # Brief fade-in only; after ~1.5s keep the cinematic readable.
                alpha = min(220, int(self.state_timer * 170))
            ...
            if self.state == "CHAPTER_2_INTRO":
                ...
                if self.state_timer > 1.0 and self.state_timer < 6.0:
                    ...
                    t3 = self.font_cinematic.render("THE FINAL STAND", True, COLORS["saffron"])
                ...
                if self.state_timer > 0.5 and self.state_timer < 5.0:
                    ...
                    t_alpha = min(255, int((self.state_timer - 0.5) * 510)) if self.state_timer < 1.5 else min(255, int((5.0 - self.state_timer) * 255))
                    t1.set_alpha(t_alpha)
                    t2.set_alpha(t_alpha)
                    t3.set_alpha(t_alpha)
                ...
                if self.state_timer > 2.0 and self.state_timer < 6.0:
                    ...
                    t2 = self.font_tutorial_text.render("THE END OF THIS STORY", True, COLORS["saffron"])
                ...
                # Keep the transition cinematic, but never leave the player on an
                # empty black screen for more than ~3 seconds.
                if self.state_timer > 0.5 and self.state_timer < 2.5:
                    text_surf = self.font_cinematic.render("THE OATH WAS KEPT.", True, COLORS["gold"])
                    t_alpha = min(255, int((self.state_timer - 0.5) * 510)) if self.state_timer < 1.5 else min(255, int((2.5 - self.state_timer) * 255))
                    text_surf.set_alpha(t_alpha)
                    surface.blit(text_surf, text_surf.get_rect(center=(LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2)))
                elif self.state_timer > 2.0 and self.state_timer < 4.0:
                    text_surf = self.font_cinematic.render("THE SON HAD FALLEN.", True, COLORS["saffron"])
                    t_alpha = min(255, int((self.state_timer - 2.0) * 510)) if self.state_timer < 3.0 else min(255, int((4.0 - self.state_timer) * 255))
                    text_surf.set_alpha(t_alpha)
                    surface.blit(text_surf, text_surf.get_rect(center=(LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2)))
                elif self.state_timer > 3.5 and self.state_timer < 5.5:
                    text_surf = self.font_cinematic.render("THE FATHER WOULD RESTORE HIM.", True, COLORS["ivory"])
                    t_alpha = min(255, int((self.state_timer - 3.5) * 510)) if self.state_timer < 4.5 else min(255, int((5.5 - self.state_timer) * 255))
                    text_surf.set_alpha(t_alpha)
                    surface.blit(text_surf, text_surf.get_rect(center=(LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2)))
                elif self.state_timer > 5.0 and self.state_timer < 7.0:
                    t1 = self.font_cinematic.render("CHAPTER III", True, COLORS["gold"])
                    t2 = self.font_cinematic.render("GANESHA REBORN", True, COLORS["ivory"])
                    t_alpha = min(255, int((self.state_timer - 5.0) * 510)) if self.state_timer < 6.0 else min(255, int((7.0 - self.state_timer) * 255))
                    t1.set_alpha(t_alpha)
                    t2.set_alpha(t_alpha)
                    surface.blit(t1, t1.get_rect(center=(LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2 - 40)))
                    surface.blit(t2, t2.get_rect(center=(LOGICAL_WIDTH//2, LOGICAL_HEIGHT//2 + 40)))
